// Initial parameters
    ...
    // Cost functions
    function total_cost(Q) {
      return 300*Q - 10*Q**2 + 0.5*Q**3;
    }
    function marginal_cost(Q) {
      return 300 - 20*Q + 1.5*Q**2;
    }
    function average_total_cost(Q) {
      return 300 - 10*Q + 0.5*Q**2;
    }

    // Demand functions
    function demand_initial(P) {
      return 30000 - 50*P;
    }
    function demand_adjusted(P, shift) {
      return (30000 + shift) - 50*P;
    }
    ...
    function updatePlots() {
      ...
      const traces = [
        // Initial Market State
        {
          ...
          type: 'scatter',
          mode: 'lines',
          name: 'Initial Demand (Q_D1)',
          line: {color: 'blue'}
        },
        {
          x: [initial_industry_quantity],
          y: [LR_price],
          type: 'scatter',
          mode: 'markers',
        ...
        {
          x: extended_quantities,
          y: sr_supply_prices,
          type: 'scatter',
          mode: 'lines',
          name: 'SR Supply Curve',
          line: {color: 'green', dash:'dot'}
        },
        // Firm-level MC and ATC
        {
          x: firm_quantities,
          y: firm_quantities.map(Q => marginal_cost(Q)),
          type: 'scatter',
          mode: 'lines',
          name: 'MC',
          line: {color: 'purple'}
        ...
        {
          x: firm_quantities,
          y: firm_quantities.map(Q => average_total_cost(Q)),
          type: 'scatter',
          mode: 'lines',
          name: 'ATC',
          line: {color: 'orange'}
